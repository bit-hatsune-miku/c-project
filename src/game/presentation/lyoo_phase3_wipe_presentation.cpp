...
constexpr float kRingStartSeconds = 0.0f;
...
constexpr char kStarTexturePath[] = "assets/combat/presentations/lyooPlotTwist/star.png";
...
LyooPhase3WipePresentation::LyooPhase3WipePresentation(
...
void LyooPhase3WipePresentation::start() {
    elapsedTime_ = 0.0f;
...
void LyooPhase3WipePresentation::update(float deltaTime) {
...
void LyooPhase3WipePresentation::preload(SDL_Renderer* renderer) {
    ensureAssetsLoaded(renderer);
}

void LyooPhase3WipePresentation::render(SDL_Renderer* renderer,
                                        int screenW,
                                        int screenH,
...
bool LyooPhase3WipePresentation::isComplete() const {
    return elapsedTime_ >= kCompleteTimeSeconds;
}

bool LyooPhase3WipePresentation::overridesCamera() const {
    return true;
}

void LyooPhase3WipePresentation::applyCameraState(Camera3D& camera) const {
    camera = frontCamera_;
...
bool LyooPhase3WipePresentation::shouldHideNonCasterCharacters() const {
    return elapsedTime_ < kApproachStartSeconds;
}

bool LyooPhase3WipePresentation::shouldRenderCasterEntity() const {
    return false;
}

bool LyooPhase3WipePresentation::shouldRenderBossEntity() const {
    return false;
}

bool LyooPhase3WipePresentation::shouldRenderAboveHud() const {
    const Phase phase = currentPhase();
    return phase == Phase::MassiveStar || phase == Phase::FullBlackout;
}

bool LyooPhase3WipePresentation::shouldBlackoutWorld() const {
    const Phase phase = currentPhase();
    return phase == Phase::RingStars || phase == Phase::SkyStars;
}

bool LyooPhase3WipePresentation::shouldRenderFloor() const {
    return true;
}

int LyooPhase3WipePresentation::consumeAbilityAudioCues() {
    const int cues = pendingAbilityAudioCues_;
    pendingAbilityAudioCues_ = 0;
    return cues;
}

int LyooPhase3WipePresentation::consumeHitEvents() {
    const int hits = pendingHitEvents_;
    pendingHitEvents_ = 0;
    return hits;
}

int LyooPhase3WipePresentation::getDamageLabelHitCount() const {
    return kFinalShakeHitBursts;
}

std::vector<PresentationAudioCommand> LyooPhase3WipePresentation::consumeAudioCommands() {
    std::vector<PresentationAudioCommand> commands;
    commands.swap(pendingAudioCommands_);
    return commands;
}

void LyooPhase3WipePresentation::ensureAssetsLoaded(SDL_Renderer* renderer) {
    if (renderer == nullptr) {
        return;
    }
...
#ifdef BATTLE_ENABLE_IMAGE
    if (SDL_Surface* surface = IMG_Load(platform::path::resolvePath(kStarTexturePath).c_str())) {
...
void LyooPhase3WipePresentation::releaseAssets() {
...
void LyooPhase3WipePresentation::seedStarTimelines() {
    ringStars_.clear();
    skyStars_.clear();
    scatterStars_.clear();
...
void LyooPhase3WipePresentation::queueAudioCommand(PresentationAudioCommandType type,
                                                   const std::string& id,
                                                   float volume) {
    pendingAudioCommands_.push_back(PresentationAudioCommand{type, id, volume});
}

LyooPhase3WipePresentation::Phase LyooPhase3WipePresentation::currentPhase() const {
...
void LyooPhase3WipePresentation::renderRingStars(SDL_Renderer* renderer,
                                                 int screenW,
                                                 int screenH,
                                                 const Camera3D& camera) {
...
void LyooPhase3WipePresentation::renderSkyStars(SDL_Renderer* renderer, int screenW, int screenH) {
...
void LyooPhase3WipePresentation::renderApproachOverlay(SDL_Renderer* renderer, int screenW, int screenH) {
    const float localT = easing::clamp01((elapsedTime_ - kApproachStartSeconds) / kApproachDurationSeconds);
...
void LyooPhase3WipePresentation::renderScatterStars(SDL_Renderer* renderer, int screenW, int screenH) {
...
void LyooPhase3WipePresentation::renderMassiveStar(SDL_Renderer* renderer, int screenW, int screenH) {
...
void LyooPhase3WipePresentation::renderFullBlackout(SDL_Renderer* renderer, int screenW, int screenH) {
    SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND);
...
void LyooPhase3WipePresentation::drawCenteredTexture(SDL_Renderer* renderer,
                                                     SDL_Texture* texture,
                                                     float centerX,
                                                     float centerY,
                                                     float width,
...
void LyooPhase3WipePresentation::drawStarSprite(SDL_Renderer* renderer,
                                                float centerX,
                                                float centerY,
                                                float size,
                                                double angleDegrees,
    ...
    SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND);
    SDL_SetRenderDrawColor(renderer, 255, 255, 255, alpha);
    const SDL_FRect vertical{centerX - size * 0.08f, centerY - size * 0.5f, size * 0.16f, size};
    const SDL_FRect horizontal{centerX - size * 0.5f, centerY - size * 0.08f, size, size * 0.16f};
    SDL_RenderFillRectF(renderer, &vertical);
    SDL_RenderFillRectF(renderer, &horizontal);
}

float LyooPhase3WipePresentation::starScaleForLifeProgress(float progress) const {
...
void LyooPhase3WipePresentation::applyShake(Camera3D& camera, float timeSeconds, float magnitude) const {
